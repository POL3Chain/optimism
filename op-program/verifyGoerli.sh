...
# Load the actual output proposal, this is the concatenation of the outputRoot, timestamp and l2BlockNumber fields
L2_OUTPUT=$(cast call --rpc-url "$L1_API" --block "$L1_HEAD" "${OUTPUT_ORACLE}" 'getL2Output(uint256)' "$L2_OUTPUT_INDEX")
echo "L2 Output: $L2_OUTPUT"

# Extract the claimed outputRoot which is the first 66 characters of the output (32 bytes in hex with the leading `0x`)
L2_CLAIM=$(echo "$L2_OUTPUT" | cut -c -66)
echo "L2 Claim: $L2_CLAIM"

# Extract the L2 block number which is the last 32 characters of the output, starting at character 131
# (2 characters for the 0x prefix, 64 for the output root and 64 for the timestamp, plus 1 because cut starts from 1)
L2_BLOCK_NUM_HEX=$(echo "$L2_OUTPUT" | cut -c 131-)
# Convert the block number to decimal
L2_BLOCK_NUM=$(( 16#$L2_BLOCK_NUM_HEX ))
